...
//        StackTraceElement[] sTE = Thread.currentThread().getStackTrace();
        ...
        if (this.handIndex != model.getHandNumber()) {
            System.out.println("Hand Indices are different ");
            System.out.println(model.getUsersHand());
        ...
        if (((model.getTrick() == null)
                || (model.trickCards() == null))
                || (model.trickCards().isEmpty())) {
            setBlank();
        } else {
            displayButtons();
            if (model.getTrick().isComplete()) {
                sleep();
            }
        }
        updatePlayerScores();
    ...
    private void removeHandButtons() {
        for (int d = model.getUsersHand().size(); d < FULL_HAND_SIZE; d++) {
    ...
    private void updateHandIcons() {
        for (int c = 0; c < model.getUsersHand().size(); c++) {
            handButtons[c].
            setIcon(cardToImage.get(model.getUsersHand().get(c)));
        }
    }

    
    /*****************************************************************
     * This helper method adds card buttons to the handPanel based
     * on the model's state.
     *****************************************************************/
    private void addHandButtons() {
        //remove all handbuttons
        for (int b = 0; b < handButtons.length; b++) {
            handPanel.remove(handButtons[b]);
        }
        //add back remaining handbuttons
        for (int c = 0; c < model.getUsersHand().size(); c++) {
            handPanel.add(handButtons[c]);
        }
    }
    ...
    private void displayButtons() {
        for (int c = model.getPlayers().indexOf(model.getTrick()
        		.getStarter());
        		c < model.getPlayers().indexOf(model.getTrick().
        				getStarter()) 
        		+ model.trickCards().size(); c++) {
            trickButtons[(c % model.getPlayers().size())].
            setIcon(cardToImage.get(model.trickCards().
            		get(c - model.getPlayers().
            				indexOf(model.getTrick()
            						.getStarter()))));
        }

        for (int c = ((model.getPlayers().indexOf(model.getTrick()
        		.getStarter()) + model.trickCards().size())
        		% model.getPlayers().size());
        		c < model.getPlayers().
        		indexOf(model.getTrick().getStarter()); c++) {
    ...
    private void setupScorePanel() {
        ...
        for (int i = 0; i < numPlayers; i++) {
            playerScores[i] = new JLabel(model.getPlayers().get(i).getName()
    ...
    protected class Listener implements ActionListener {
        ...
        public void actionPerformed(final ActionEvent event) {
            System.out.println("Got into AE");
            ...
            if (model.isUsersTurn()) {
                for (int h = 0; h < model.getUsersHand().size(); h++) {
                    if (handButtons[h] == event.getSource()) {
                        if ((!model.getTrick().getTrick().isEmpty()) 
                        		&& (model.canFollowSuit())) {
                            checkCard(h);
                        } else {
        ...
        private void checkCard(final int h) {
            if ((model.getTrick().getLed().getSuit()
                    == model.getUsersHand().get(h).
                    getSuit())
                    || (model.getTrick().getTrick().isEmpty())) {
                selectedCard = model.getUsersHand().get(h);
            }
        }
    }
    ...
    /******************************************************************
     * This class is part of the Controller portion of the 
     * Model-View-Controller design for a CanadianSalad simulator.
     * It adjusts game play based on the the CanadianSaladModel's
     * state.
     *
     * @author Andrew Weston and Christian Thompson
     * @version Summer 2019
    ...
    public class ModelListener implements ChangeListener {
        ...
        /**************************************************************
         * This method defines interactions between the Panel and the
         * CanadianSaladModel game logic.
         *
         * @param e is an ChangeEvent from the CanadianSaladModel.
         *************************************************************/
    ...
    /******************************************************************
     * This method causes the program to delay play to simulate
     * other players taking their turns.
     *****************************************************************/
    private void sleep() {
        try {
    ...
    /******************************************************************
     * This method causes the program to delay panel update after
